...
def display_plot(ax, fig, i, theta, phi, dipole_angle, total_tau, sticking_force, r, gap, seed, dtheta, path, N, Q):
    ...
    ax2.clear()
    ax3.clear()
    ax3.set_ylim(-max_tau, max_tau)
    ax3.set_xlim(0, 360)
    ax2.set_ylim(max_F, 0)
    ax2.set_xlim(0, 360)
    ax2.set_xlabel(u'\u0394\u03B8 (\u00B0)')
    ax3.set_ylabel(u'\u03C4 (mNmm)')
    # ax2.set_xlabel(u'\u0394\u03B8 (\u00B0)')
    ax2.set_ylabel('F (mN)')
    ...
    ax3.plot(dipole_angle[:i]*180/np.pi, 1e6*total_tau[:i], '-c', linewidth=5)
    ax3.plot(dipole_angle[i]*180/np.pi, 1e6*total_tau[i],
             color='yellow', marker='.', markeredgecolor='black', markeredgewidth=1.5, markersize=10)
    ax2.plot(dipole_angle[:i]*180/np.pi, 1e3 *
             sticking_force[:i], '-m', linewidth=5)
    ax2.plot(dipole_angle[i]*180/np.pi, 1e3*sticking_force[i],
             color='yellow', marker='.', markeredgecolor='black', markeredgewidth=1.5, markersize=10)
    ...
    fig.savefig(path + '/torque_model_N' + str(N) + '_Q' + str(Q) +
                '_seed' + str(seed) + '_' + str(i) + '.png', format='png')
...
def display_anim(N, dipole_angle, total_tau, sticking_force, r, gap, path, seed=3, dtheta=0.05, DPI=128, final=False):
    ...
    ax3 = fig.add_subplot(gs[9:12, 4:18])
    ax2 = fig.add_subplot(gs[13:16, 4:18])
    ax = (ax1, ax2, ax3)

    theta, phi = angles(N, seed=seed)

    if final:
        display_plot(ax, fig, len(dipole_angle)-1, theta, phi, dipole_angle,
                     total_tau, sticking_force, r, gap, seed, dtheta, path, N, Q)
    else:
        for i in range(len(dipole_angle)):
            display_plot(ax, fig, i, theta, phi, dipole_angle, total_tau,
                         sticking_force, r, gap, seed, dtheta, path, N, Q)
            theta = theta+dtheta
...
if __name__ == "__main__":
    ...
    path = os.environ['USERPROFILE'] + \
        '/OneDrive - The University of Nottingham/Documents/Papers/Charge/Static_Charging/Figures/Figure4/'
    ...
    dQ = 0
    new_Q = Q
    dtheta = 0.05  # increment to calc torque at
    gap = 0.5e-3  # thickness of glass slide
    L = 3*r
    scale = 1000  # Plot everything in mm
    num_charges = [20]  # [2, 5, 10, 20, 50, 100, 200, 500, 1000]
    num_seeds = 3  # 500
    num_view = 3  # This is the seed you want to view

    for N in [20]:#num_charges:
        print(N)
        ...
        for i in range(num_seeds, num_seeds+1, 1):
            print(i)
